...
class ControllerNode(object):
    ...
    def tcallback_control(self, event):
        ...
        if self.Int_Err is not None:
            self.X = np.array([self.x, self.y, self.theta, self.v, self.vn, self.w])[np.newaxis].T
            dist = np.sqrt((self.x - self.x_goal)**2 + (self.y - self.y_goal)**2)
            if dist <= 0.1:
    ...
    def callback_localization(self, data):
        ...
        self.x = 2.75 - self.robot_pose[1]  # Position X
        self.y = 0.85 - self.robot_pose[2]  # Position Y
        self.theta = wrap_angle(3.14 - self.robot_pose[3])  # Angle Theta
